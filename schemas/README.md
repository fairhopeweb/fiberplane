...
3. Commit the changes to the `schema` and `fiberplane-api-client`
   - This important for a clean diff
4. Build the api_client: `cargo build -p fiberplane-api-client`
   - **If it doesn't compile we need to make a patch 😔:**
     1. Make your changes in the `fiberplane-api-client/` folder. Most likely `fiberplane-api-client/src/apis/default_api.rs`
     2. Create a patch of the changes: `git diff -p fiberplane-api-client/ > schemas/patches/your_patch_name.patch`
